...
#include <LiquidCrystal.h>   // Appeler la librairie nécessaire pour l'écran
#include <Servo.h>          // Appeler la librairie nécessaire pour les servomoteurs


//Assignation des broches pour l'écran //

LiquidCrystal lcd(8, 9, 4, 5, 6, 7);  // Définir les broches utilisées par l'écran (LED rétoéclairage sur pin 10)
...
int tolerancev = 50; //détermine la tolérance entre la différence des valeurs des LDR hd et bd
int toleranceh = 50; //détermine la tolérance entre la différence des valeurs des LDR hd et hg
...
void setup()   // le "setup" est exécuté une seule fois au début du programme
{
  ...
  lcd.begin(16, 2);    // définition du type d'affichage utilisé (x=nombre de case, y= nombre de caractère)
  Serial.begin(9600);

  lcd.clear();         // effacement de l'écran
  lcd.setCursor(0, 0); // positionnement du curseur en haut à gauche (x=nombre de case, y= nombre de caractère)
  lcd.print("Solar tracker ");   // affichage sur l'écran du mot "Solar tracker " 
  delay(500);
}
...
/* Affichage*/
     ...
     lcd.clear();         // effacement de l'écran
     lcd.setCursor(0, 0); // positionnement du curseur en haut à gauche (x=nombre de case, y= nombre de caractère)
     lcd.print("HG ");   // affichage sur l'écran du mot "HD " 
     lcd.print(hg);      // affichage de la valeur mesurée de la variable hd
     lcd.print(" HD ");   // affichage sur l'écran du mot "HG "
     lcd.print(hd);      // affichage de la valeur mesurée de la variable hd
     lcd.setCursor(0, 1); // positionnement du curseur en haut à gauche (x=nombre de case, y= nombre de caractère)
     lcd.print("BG ");   // affichage sur l'écran du mot "BD "
     lcd.print(bg);      // affichage de la valeur mesurée de la variable hd
     lcd.print(" BD ");   // affichage sur l'écran du mot "BG "
     lcd.print(bd);      // affichage de la valeur mesurée de la variable hd
     delay(10);   // délai de x0 secondes qui permet de stabiliser l'écran ACL   
}
