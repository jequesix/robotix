...
void CrcXbee::UpdateData(uint8_t batStatus)
{
    ...
    if (resp.isAvailable()) {
        ...
        if (resp.getApiId() == ZB_RX_RESPONSE) {
            ...
            Serial.println(rx.getValue()[0], HEX);
            Serial.println(rx.getValue()[1], HEX);
            Serial.println(rx.getValue()[2], HEX);
            Serial.println(rx.getValue()[3], HEX);
            Serial.println(val, HEX);
            Serial.println((char)rx.getCommand()[1]);
            ...
            Serial.println(_addr.getMsb(), HEX);
            Serial.println(_addr.getLsb(), HEX);
...
void CrcXbee::ResetState() { _state = {}; }
