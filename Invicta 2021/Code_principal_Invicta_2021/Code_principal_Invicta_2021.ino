...
#define Moteur_hauteur CRC_PWM_2
...

{
  ...
  if (millis() - previousMillis2 > intervalle2)
  {
    CrcLib::SetPwmOutput(Moteur_extension, 0);
    piton2 = LOW;
  }
...
void Banebot_hauteur(char puissance, const long intervalle)
{

  piton = HIGH;
  if (piton == HIGH)
  {
    CrcLib::SetPwmOutput(Moteur_hauteur, puissance);
...
void loop()
{
  ...
  if (CrcLib::GetDigitalInput(Bouton_Power) == LOW)
  {
    ...
    while (1 == 1)
    {
      ...
      if (step1 == HIGH)
      {
        rotation_encodeur(-1000, 40);

        if (GLOBALE <= -980 & GLOBALE >= -1100)
        {
      ...
      if (step3 == HIGH)
      {
        ...
        if (GLOBALE >= 980 & GLOBALE <= 1100)
        {
          step0 = LOW; step1 = LOW; step2 = LOW; step3 = LOW; step4 = LOW; step5 = LOW; step6 = LOW;
